...
(function ($) {
    ...
    let _getRegexName = function (widgetOptions) {
        return new RegExp(widgetOptions.regexName, 'i');
    };

    let _getRegexId = function (widgetOptions) {
        return new RegExp(widgetOptions.regexID, 'i');
    };
    ...
    var _addItem = function (widgetOptions, e, $elem) {
        ...
        if (count < widgetOptions.limit) {
            let $toclone = widgetOptions.template;
            let $newclone = $toclone.clone(false, false);
    ...
    var _updateAttrID = function ($elem, index) {
        ...
        if (id !== undefined) {
            var matches = id.match(_getRegexId(widgetOptions));
    ...
    var _updateAttrName = function ($elem, index) {
        ...
        if (name !== undefined) {
            var matches = name.match(_getRegexName(widgetOptions));
            if (matches && matches.length === 4) {
    ...
    var _restoreKrajeeDepdrop = function ($elem, widgetOptions) {
        var configDepdrop = $.extend(true, {}, eval($elem.attr('data-krajee-depdrop')));
        var inputID = $elem.attr('id');
        var matchID = inputID.match(_getRegexId(widgetOptions));

        if (matchID && matchID.length === 4) {
            for (index = 0; index < configDepdrop.depends.length; ++index) {
                var match = configDepdrop.depends[index].match(_getRegexId(widgetOptions));
    ...
    var _restoreSpecialJs = function (widgetOptions) {
        ...
        if ($hasDateControl.length > 0) {
            $hasDateControl.each((i, e) => {
                let id = $(e).attr('id');
                let dcElementOptions = eval($(e).attr('data-krajee-datecontrol'));
                let oldValue = $(e).val();
                ...
                let idSave = $(e).parent().next().attr('id');
                cdNewOptions.idSave = idSave;
                $(e).prop('name', id);
                $(e).parent().kvDatepicker(eval($(e).attr('data-krajee-kvdatepicker')));
                $(e).removeAttr('value name data-krajee-datecontrol');
                $(e).datecontrol(cdNewOptions);
                $(e).val(oldValue);
                // }
            })
        }
        ...
        if ($hasDepdrop.length > 0) {
            $hasDepdrop.each(function () {
                if ($(this).data('select2') === undefined) {
                    $(this).removeData().off();
                    $(this).unbind();
                    _restoreKrajeeDepdrop($(this), widgetOptions);
                }
            });
        }
